...
> [!NOTE]
> Tem de utilizar licenças do Office 365 ProPlus para ativar as aplicações do Office 365 ProPlus implementadas através do Microsoft Intune. A edição de negócios do Office 365 é suportada pela Intune, no entanto deve configurar o conjunto de aplicações da edição de negócios do Office 365 usando dados XML. Para mais informações, consulte o conjunto de [aplicações Configure utilizando dados XML](apps-add-office365.md#step-2---option-2-configure-app-suite-using-xml-data).
...
1. Adicionei a sua configuração XML.
    ...
    > [!NOTE]
    > O ID do produto pode ser Business (`O365BusinessRetail`) ou Proplus (`O365ProPlusRetail`). No entanto, só é possível configurar o conjunto de aplicações da edição de negócios do Office 365 utilizando dados XML. 
...
- Os requisitos de rede intune e office 365 são cumpridos e as gamas IP relacionadas são acessíveis com base nos seguintes artigos:
  ...
  - [Intervalos de endereços de escritório 365 URLs e IP](https://docs.microsoft.com/office365/enterprise/urls-and-ip-address-ranges)
