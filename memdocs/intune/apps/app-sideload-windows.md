...
Como administrador intune, pode implementar aplicações universais de linha de negócio (LOB) para o Windows 8.1 Desktop ou Windows 10 Desktop & dispositivos Mobile, incluindo a aplicação Portal da Empresa. Para implementar *aplicações .appx* para windows 8.1 Desktop ou Windows 10 Desktop & Dispositivos Móveis pode utilizar certificado de assinatura de código a partir de uma autoridade de certificação pública já confiável pelos seus dispositivos Windows, ou pode usar a sua própria autoridade de certificados.
...
3. Selecione **Todos os dispositivos Windows 10** como a **Plataforma** e, em seguida, a **Arquitetura** adequada e transfira. Não é preciso um ficheiro de licença de aplicação para esta aplicação.
   ![Imagem do Windows 10 X86 Detalhes do pacote para download](./media/app-sideload-windows/Win10CP-all-devices.png)
4. Descarregue todos os pacotes em "Quadros Obrigatórios". Isto deve ser feito para arquiteturas x86, x64, ARM e ARM64 – resultando num total de 9 embalagens, como mostrado abaixo.
...
### <a name="how-do-i-update-the-company-portal-on-my-users-devices-if-they-have-already-installed-the-older-apps-from-the-store"></a>Como atualizo o Portal da Empresa nos dispositivos dos meus utilizadores se já instalaram as aplicações mais antigas a partir da loja?
...
A nossa trajetória de migração recomendada é eliminar a implementação da aplicação Portal da Empresa Windows 8.1, definindo a ação de implementação para "Desinstalar". Em seguida, a aplicação Portal da Empresa do Windows 10 pode ser implementada através de qualquer uma das opções acima.  
...
A nossa via de migração recomendada é eliminar a implementação existente para a aplicação Portal da Empresa Windows Phone 8.1 ou para a aplicação Portal da Empresa Windows 8.1, definindo a ação de implementação para "Desinstalar". Em seguida, a aplicação Portal da Empresa do Windows 10 poderá ser implementada normalmente.  
