...
- Instale as seguintes aplicações neste dispositivo:  
  ...
  - Instale o controlador de ferramentas de [embalagem MSIX](/windows/msix/packaging-tool/tool-known-issues#frameworks-and-drivers)<!--SCCMDocs-pr issue #3091-->  
...
Os dispositivos windows 10 não necessitam de uma chave de sideloading para instalar aplicações de linha de negócio. No entanto, para permitir a sideloading no Windows, a chave `HKEY_LOCAL_MACHINE\Software\Policies\Microsoft\Windows\Appx\AllowAllTrustedApps` de registo deve ter um valor de **1**.  
